### To test scenarios with each agent running its own decentralised, independent macbf, with option to inject noise into states of K nearest neighbours (simulate real-world imperfect information).
### Limit number of agents, N, to reduce compute load. N times as heavy as original macbf. 
...
def parse_args():
    ...
    # --noise is the standard deviation of Gaussian noise applied to the position and velocity of the robots, in cm and cm/s respectively. This represents onboard sensor inaccuracies.
    parser.add_argument('--noise', type=float, default=0)
    # --dropout is the percentage chance that the position and velocity of another robot is missing as input to compute the next control. This represents comms imperfections. 
...
def main():
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        for i in range(config.INNER_LOOPS):
            if args.noise != 0:
                # compute the Gaussian noise to the agent states
                s_noise = np.random.normal(0, args.noise, [args.num_agents, 4])
            else:
                s_noise = np.zeros([args.num_agents, 4])
            # compute the noisy state input
            s_np_noisy = s_np + s_noise

            a_network = np.zeros([args.num_agents, 2])
            # for each agent, compute the control input 
            for n in range(args.num_agents):
                # compute the state input with random dropout on k nearest neighbour states

                # compute the control input. Extract only a (control action) and acc_list (accuracies) from the network output.
                a_network_n, acc_list_np = sess.run([a, acc_list], feed_dict={s: s_np_noisy, g: g_np})
                # consolidate the control input for all agents as a_network
                a_network[n] = a_network_n[n]
            ...
            if args.vis:
                # break if all agents are already very close to their goals(DIST_MIN_CHECK / 3)
                if np.amax(
                    np.linalg.norm(s_np[:, :2] - g_np, axis=1)
                    ) < config.DIST_MIN_CHECK / 3:
                    time.sleep(1)
                    break
                # if the (mean) agents are very close to their goals (DIST_MIN_CHECK / 2), safely switch to LQR
                if np.mean(
                    np.linalg.norm(s_np[:, :2] - g_np, axis=1)
                ...
                # break if the (mean) agents are close to their goals (DIST_MIN_CHECK)
                if np.mean(
                    np.linalg.norm(s_np[:, :2] - g_np, axis=1)
        ...
        s_np, g_np = np.copy(s_np_ori), np.copy(g_np_ori)
        for i in range(config.INNER_LOOPS):
            # TODO: ensure s_np_noisy is identical for macbf and LQR
            if args.noise != 0:
                # compute the Gaussian noise to the agent states
                s_noise = np.random.normal(0, args.noise, [args.num_agents, 4])
            else:
            ...
            K = np.eye(2, 4) + np.eye(2, 4, k=2) * np.sqrt(3)
            s_ref = np.concatenate([s_np_noisy[:, :2] - g_np, s_np_noisy[:, 2:]], axis=1)
            a_lqr = -s_ref.dot(K.T)
